"""V6.0 신경망 — 행동 헤드가 직사각형 '내부'를 보게 만든다.

V5.0 의 헤드는 두 꼭짓점 임베딩의 내적으로 로짓을 만들었다. 이 형태는 원리상
"이 사각형 안에 무엇이 들어있는가"를 표현할 수 없는데, 이 게임에서 한 수의
가치를 결정하는 것이 정확히 그 정보다 (env.py 의 측정 참고).
...
WIDTH = 96          # 잔차 블록 채널 수
N_BLOCKS = 6        # 잔차 블록 개수 (수용영역 13x13 + 전역 브랜치)
EMBED_DIM = 64      # 셀별 임베딩 차원
...
class PooledCriticExtractor(nn.Module):
    """정책에는 특징을 그대로, 가치망에는 셀 임베딩을 풀링해서 넘긴다."""

    def __init__(self, features_dim: int, cell_dim: int, embed_dim: int, hidden: int = VALUE_HIDDEN):
        super().__init__()
        self.cell_dim, self.embed_dim = cell_dim, embed_dim
    ...
    def forward_critic(self, features: torch.Tensor) -> torch.Tensor:
        ...
        pooled = torch.cat([cells.mean(dim=2), cells.amax(dim=2)], dim=1)
...
class RectangleMaskablePolicy(MaskableActorCriticPolicy):
    ...
    def _build_mlp_extractor(self) -> None:
        ...
        self.mlp_extractor = PooledCriticExtractor(
            fe.features_dim, fe.cell_dim, fe.embed_dim,  # type: ignore[arg-type]
        )
