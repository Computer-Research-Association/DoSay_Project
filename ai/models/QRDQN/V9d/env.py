"""QR-DQN V9d 환경 — 분포 학습 + 후반 커리큘럼. 두 베팅을 겹친 조합이다.

V9a(분포만) 와 V9b(커리큘럼만) 가 각각 효과가 있다면 이 조합이 가장 높아야 한다.
셋을 비교하면 두 방향이 더해지는지 겹치는지도 같이 드러난다.

**후반 커리큘럼.** 100판 벤치마크에서 모든 모델이 사과를 46~51개 남기고 끝난다.
점수를 잃는 곳은 초반이 아니라 판이 말라가는 후반인데, 후반 상태는 에피소드
꼬리에만 나와 경사의 20%도 못 받는다. 학습할 때만 에피소드 절반을 이미 진행된
판에서 시작해 후반 연습량을 늘린다. 손으로 만든 후반 규칙이 아니라 무작위
합법수로 도달한 실제 분포를 쓴다.

평가는 make_env() 를 쓰므로 언제나 꽉 찬 판에서 시작한다. 탐색은 끈다.
"""
