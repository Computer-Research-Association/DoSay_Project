"""V6.0 학습 스크립트 — 아직 실행되지 않았다.

이 폴더에 zip 이 없으므로 measure.py 의 목록에 나타나지 않는다.
아래를 실행해 체크포인트를 만들면 그때부터 선택 가능해지고,
동시에 env.py / model.py 는 동결 대상이 된다.

실행:  python ai/models/version/V6.0/train.py
"""
