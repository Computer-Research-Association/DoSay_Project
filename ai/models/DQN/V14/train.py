"""DQN V14 — **빔 교사 정책반복.** 빔이 찾은 수순을 신경망에 증류한다.

    데이터:  빔(폭 32, 학습된 가치)으로 한 판을 끝까지 계획하고 그대로 둔다
    정책:    그 수순 50개를 margin 손실로 (배포용 1회 forward 모델)
    가치:    그 판의 실제 결과를 회귀로 (다음 바퀴의 빔이 더 좋아진다)
    반복:    가치↑ -> 빔↑ -> 데이터↑ -> 가치↑

──────────────────────────────────────────────────────────────────────────
왜 이 교사인가 (V13 의 롤아웃 교사와 무엇이 다른가)
──────────────────────────────────────────────────────────────────────────
V13 은 **자기 정책으로 롤아웃**해서 교사를 만들었다. 정책이 101점이면 교사도
그 언저리다 — 부트스트랩이 걸리지 않는다. 실제로 V13L(정책 단독)은 101.15 였다.

**빔은 정책이 아니라 가치 헤드만 쓴다.** 그래서 정책이 아무리 나빠도 교사는 강하다.
V12b 가중치로 워밍스타트하면 **1스텝째부터 교사가 123.9** 다.

| 교사 | 점수 | 초/수 | 판당 라벨 |
|---|---|---|---|
| V13 의 롤아웃 | 118.46 | 0.22 | 1개 (0.22초) |
| **빔 W=32** | **123.9** | **0.05** | **50개 (2.7초)** |

**더 강하고, 4배 싸고, 샘플 효율이 4배다.**

──────────────────────────────────────────────────────────────────────────
왜 빔이 한 판에 한 번이면 되는가
──────────────────────────────────────────────────────────────────────────
판이 정해지면 이 게임에는 무작위성이 전혀 없다. 처음에 끝까지 계획한 수순을 그대로
따르는 것과 매 수 재계획하는 것이 **정확히 같다.** 그래서 ε 탐험도 쓰지 않는다 —
계획에서 벗어나면 캐시가 무효가 될 뿐 얻는 것이 없고, 다양성은 판이 제공한다.

──────────────────────────────────────────────────────────────────────────
배포 (같은 가중치, model.py 의 MODE 만 다르다)
──────────────────────────────────────────────────────────────────────────
    DQN/V14L   MODE="policy"      1회 forward        0.006초/수
    DQN/V14A   MODE="afterstate"  깊이 1             0.009초/수
    DQN/V14    MODE="beam"        빔 W=32            0.05초/수

세 폴더에 같은 체크포인트를 복사해 넣고 measure.py 로 재면 된다 (재학습 불필요).
**어느 선까지가 쓸 만한지는 세 숫자를 보고 정하면 된다.**

기준: V1.0 114.49 (1 forward 역대 최고) / 1수앞 손규칙 116.49 /
V12b+빔W=32 123.9 (학습 0) / 어닐링 ~137 / 이론상 최대 137~140
"""
