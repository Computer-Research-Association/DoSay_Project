"""DQN V13L 신경망 — **V13 과 가중치가 완전히 같다. 상수 하나만 다르다.**

    (V13)   ROLLOUT_TOPK = 16   후보 16개를 끝까지 두어 보고 고른다 (무거운 모델)
    (V13L)  ROLLOUT_TOPK = 0    정책 로짓 argmax. **1회 forward.** (경량 모델)

학습은 V13 에서만 한다. 여기는 그 체크포인트를 이름만 바꿔 복사해 넣고 측정하는
용도다 (V9c/V9cB 와 같은 방식 — 탐색 설정은 추론 시점 설정이라 체크포인트에
들어가지 않으므로 재학습 없이 로드된다).

**이 둘의 점수 차이가 곧 "롤아웃이 가중치로 얼마나 흡수됐는가"** 다.
학습 로그의 `train/teacher_agreement`(정책 argmax 가 롤아웃의 선택과 일치하는 비율)
가 그 지표의 학습 중 버전이다.

아래는 V13 model.py 의 설명 그대로다.
...
ROLLOUT_TOPK = 0         # **경량 모델** — 롤아웃 없이 정책 로짓 argmax, 1회 forward
ROLLOUT_SAMPLES = 1
