"""DQN V14L 신경망 — **V14 와 가중치가 완전히 같다. MODE 상수만 다르다.**

    이 폴더: MODE = "policy"  -> 1회 forward (정책 로짓 argmax). **경량 모델.** 0.006초/수

학습은 V14 에서만 한다. 여기는 그 체크포인트를 이름만 바꿔 복사해 넣고
측정하는 용도다 (V9c/V9cB 와 같은 방식 — 배포 모드는 추론 시점 설정이라
체크포인트에 들어가지 않으므로 재학습 없이 로드된다).

아래는 V14 model.py 의 설명 그대로다.
...
MODE = "policy"          # "policy" | "afterstate" | "beam"
BEAM_WIDTH = 32
