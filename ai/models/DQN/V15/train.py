...
class GuidedBeamDQN(DQN):
    ...
    def train(self, gradient_steps: int, batch_size: int = 100) -> None:
        ...
        pol_losses, agrees, top8, value_losses, value_maes, entropies = (
            [], [], [], [], [], [])
        for _ in range(gradient_steps):
            ...
            with torch.no_grad():
                ...
                # 목표가 분포라서 **교차엔트로피의 바닥은 0 이 아니라 목표의
                # 엔트로피**다. 그냥 policy_loss 만 보면 "안 내려간다" 고 오독한다.
                # KL = policy_loss - entropy 가 0 으로 가야 맞는 것이다.
                entropies.append(
                    -(target * target.clamp_min(1e-9).log()).sum(dim=1).mean().item())
        ...
        # **이 줄이 0 으로 가야 증류가 되는 것이다.** policy_loss 자체는 목표
        # 엔트로피(~1.8) 아래로 내려갈 수 없다.
        self.logger.record("train/policy_kl",
                           float(np.mean(pol_losses) - np.mean(entropies)))
        self.logger.record("train/target_entropy", float(np.mean(entropies)))
