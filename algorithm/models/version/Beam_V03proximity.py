...
@registry.heuristic(weight=1.0)
def feature_nine_one_proximity(board: Board) -> float:
    g = board.grid
    nines = np.argwhere(g == 9)
    ones  = np.argwhere(g == 1)
    if len(nines) == 0:
        return 1.0                      # 9 다 지움 = 최고
    if len(ones) == 0:
        return -1.0                     # 짝지을 1이 아예 없음
    # 각 9 → 최근접 1 거리 (맨해튼)
    d = np.abs(nines[:, None, :] - ones[None, :, :]).sum(axis=2).min(axis=1)
    max_d = g.shape[0] + g.shape[1]      # 정규화
    return float(np.mean(1 - d / max_d)) # 9들이 1에 가까울수록 →1
