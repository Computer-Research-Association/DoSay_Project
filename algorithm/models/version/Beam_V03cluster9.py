...
@registry.heuristic(weight=1.0)
def feature_nine_cluster(board: Board) -> float:
    nine = (board.grid == 9).astype(np.int8)
    if int(nine.sum()) == 0:
        return 0.0
    win = sliding_window_view(nine, (3, 3)).sum(axis=(2, 3))
    return -int(win.max()) / 9.0         # 9가 좁게 몰릴수록 페널티
